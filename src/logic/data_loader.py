    ...
    async def _load_from_api(self):
        ...
        async with ClientSession(connector=self.conn) as session:
            async with session.get(url, params=params) as resp:
                ...
                self.log.info(self, f'Requesting {url} with params {params} -> status: {status}')
        return body, status
...
class CachedDataLoader(DataLoader):
    ...
    def _dump_cache(self):
        cache_data = dict(cross_table=self.data.data, status=self.data.status, timestamp=self.timestamp)
    ...
    def _pick_cache(self):
        ...
        self.timestamp = cache_data['timestamp']
